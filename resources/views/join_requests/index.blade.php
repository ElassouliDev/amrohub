        ...
        <table class="min-w-full leading-normal">
            <thead>
                <tr>
                    ...
                    <th class="px-5 py-3 border-b-2 border-gray-200 bg-gray-100 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">University Id</th>
            ...
            <tbody>
                @foreach($joinRequests as $request)
                    <tr>
                        ...
                        <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">{{ $request->phone }}</td>
                        <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">{{ ucfirst($request->gender) }}</td>
                        <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">{{ ucfirst($request->type) }}</td>

                        <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">{{ $request->university }}</td>
                        <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">{{ $request->specialization }}</td>
                        <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">{{ $request->university_id }}</td>
                        <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">{{ $request->plan->title ?? 'N/A' }}</td>
                        <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">{{ $request->created_at }}</td>
                        ...
                        <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm flex space-x-2">
                            @if($request->status === 'pending')
                                <form action="{{ route('join-requests.update-status', $request) }}" method="POST">
                                    @csrf
                                    @method('PUT')
                            ...
                            @endif
                                <a href="{{route('join-requests.edit',$request)}}" class="text-orange-600 hover:text-orange-900">Edit</a>
